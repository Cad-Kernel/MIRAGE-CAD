...
$DataDir = Join-Path $ProjectRoot "data\smoke500"
$FeatureDir = Join-Path $DataDir "step_features"
$LocalDataDir = "C:\tmp\MIRAGE\smoke500"
...
foreach ($split in $splits) {
    ...
    if (!(Test-Path -LiteralPath $InputJsonl)) {
        throw "Missing $InputJsonl. Run this first in WSL: cd ~/workspace/MIRAGE/src && bash scripts/prepare_500.sh"
    }
