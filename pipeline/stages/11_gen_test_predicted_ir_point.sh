#!/bin/bash
# Phase 6 / Stage 3 formal eval: generate predicted_ir for the full 2.5K test set, point-cloud query.
set -euo pipefail
...
python training_25k/scripts/gen_predicted_ir.py \
  ...
  --output-jsonl outputs/lora_ir_25k/predicted_ir_test_point.jsonl \
  --max-length 1536 --max-new-tokens 1536

echo "=== Wrote outputs/lora_ir_25k/predicted_ir_test_point.jsonl ==="
