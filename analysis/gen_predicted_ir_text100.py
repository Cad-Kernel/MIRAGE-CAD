"""Stage 3: generate predicted_ir for 100 held-out text-query test samples.

Mirrors scratch/gen_predicted_ir_point100.py, parameterized for text modality.
"""
import json
import sys
...
payload = torch.load(Path("outputs/prior_text_5k/best.pt"), map_location="cpu", weights_only=False)
...
out_path = Path("outputs/lora_ir_5k/predicted_ir_text100.jsonl")
max_length = 2048
max_new_tokens = 1536
...
with open(out_path, "w", encoding="utf-8", newline="\n") as f:
    for i, row in enumerate(rows):
        ...
        with torch.no_grad():
            z_ir_hat = prior(z_m)
        prompt = build_ir_prompt(row, "text", retrieved_ir=None, point_xyz=None)
