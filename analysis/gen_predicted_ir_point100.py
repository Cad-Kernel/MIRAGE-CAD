"""Stage 3: generate predicted_ir for 100 held-out point-cloud test samples.

Mirrors gen_scripts/generate_predicted_ir.py's IR-generation core, but runs
against the TEST split (that script hard-blocks non-train rows since it's
meant for Stage 4b cache building, not eval).
"""
import json
import sys
...
payload = torch.load(Path("outputs/prior_point_5k/best.pt"), map_location="cpu", weights_only=False)
...
out_path = Path("outputs/lora_ir_5k/predicted_ir_point100.jsonl")
max_length = 2048
max_new_tokens = 1536
...
with open(out_path, "w", encoding="utf-8", newline="\n") as f:
    for i, row in enumerate(rows):
        ...
        z_m = aligner.encode_point(torch.tensor(pts[None], dtype=torch.float32, device=device))
        with torch.no_grad():
            z_ir_hat = prior(z_m)
        prompt = build_ir_prompt(row, "point", retrieved_ir=None, point_xyz=pts)
