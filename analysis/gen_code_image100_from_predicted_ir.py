"""Stage 4b: generate program.py for 100 image-query test samples from
(P1a-repaired) predicted_ir. Mirrors scratch/gen_code_text100_from_predicted_ir.py,
parameterized for image modality.
"""
import json
import sys
...
ir_rows = read_jsonl(Path("outputs/lora_ir_5k/predicted_ir_image100_p1a.jsonl"))
test_rows = {r["sample_id"]: r for r in read_jsonl(Path("data/smoke5k/test.jsonl"))[:100]}

out_path = Path("outputs/qwen25_coder_1_5b_program_5k_stage4b/gen_image100_from_predicted_ir.jsonl")
with open(out_path, "w", encoding="utf-8", newline="\n") as f:
    for i, ir_row in enumerate(ir_rows):
        ...
        prompt = build_program_prompt(row, "image", predicted_ir, point_xyz=None)
