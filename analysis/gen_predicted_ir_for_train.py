"""Generate predicted_ir for a slice of the TRAIN split (Stage 3 pass1 only,
no LoRA-Code) — used to build the Stage 4b mixed fine-tuning set. Train split
only, so there is no val/test leakage into training.
"""
import json
import sys
...
args = Args()
rows = read_jsonl(Path("data/smoke5k/train.jsonl"))[START : START + N]
out_path = Path("outputs/qwen25_coder_1_5b_program_5k/predicted_ir_train_subset.jsonl")
with open(out_path, "w", encoding="utf-8", newline="\n") as f:
    for i, row in enumerate(rows):
        ...
        f.write(json.dumps({"sample_id": row["sample_id"], "predicted_ir": predicted_ir}, ensure_ascii=False) + "\n")
        if (i + 1) % 20 == 0:
